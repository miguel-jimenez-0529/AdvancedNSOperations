...
class EarthquakesTableViewController: UITableViewController {
  ...
  var fetchedResultsController: NSFetchedResultsController<Earthquake>?
    ...
    let operationQueue = AppleOperationQueue()
    
    // MARK: View Controller
    
    override func viewDidLoad() {
        super.viewDidLoad()
      tableView.refreshControl?.addTarget(self, action: #selector(self.startRefreshing(sender:)), for: .valueChanged)
        let operation = LoadModelOperation { context in
            // Now that we have a context, build our `FetchedResultsController`.
            DispatchQueue.main.async {
              let request = Earthquake.fetchRequest() as! NSFetchRequest<Earthquake>
  ...
  override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        */
        ...
        let operation = AppleBlockOperation {
    ...
    @objc func startRefreshing(sender: UIRefreshControl) {
        getEarthquakes()
    }
