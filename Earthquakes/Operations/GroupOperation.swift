...
*/
class GroupOperation: AppleOperation {
  private let internalQueue = AppleOperationQueue()
  private let startingOperation = AppleBlockOperation(block: {_ in })
  private let finishingOperation = AppleBlockOperation(block: {_ in })
...
extension GroupOperation: OperationQueueDelegate {
    ...
    final func operationQueue(operationQueue: AppleOperationQueue, willAddOperation operation: Operation) {
    ...
    final func operationQueue(operationQueue: AppleOperationQueue, operationDidFinish operation: Operation, withErrors errors: [Error]) {
        aggregatedErrors.append(contentsOf: errors)
        if operation === finishingOperation {
          internalQueue.isSuspended = true
          finish(errors: aggregatedErrors as [NSError])
        }
        else if operation !== startingOperation {
          operationDidFinish(operation: operation, withErrors: errors as [NSError])
        }
    }
}
