...
*/
class AppleOperation: Operation {
  ...
  override var isReady: Bool {
        switch state {
            ...
            case .Initialized:
                // If the operation has been isCancelled, "isReady" should return true
              return isCancelled
  ...
  override var isFinished: Bool {
    ...
    private func evaluateConditions() {
      ...
      OperationConditionEvaluator.evaluate(conditions: conditions, operation: self) { (_) in
  ...
  override func addDependency(_ operation: Operation) {
    ...
    override final func start() {
        ...
        // If the operation has been isCancelled, we still need to enter the "Finished" state.
      if isCancelled {
            finish()
        }
    }
    ...
    */
    final func finishWithError(error: Error?) {
        if let error = error {
          finish(errors: [error as NSError])
        }
        else {
            finish()
        }
