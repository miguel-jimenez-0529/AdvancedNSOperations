...
@objc protocol OperationQueueDelegate: NSObjectProtocol {
  @objc optional func operationQueue(operationQueue: AppleOperationQueue, willAddOperation operation: Operation)
  @objc optional func operationQueue(operationQueue: AppleOperationQueue, operationDidFinish operation: Operation, withErrors errors: [Error])
}
...
*/
class AppleOperationQueue: OperationQueue {
    weak var delegate: OperationQueueDelegate?
    
  override func addOperation(_ operation: Operation) {
        if let op = operation as? AppleOperation {
            // Set up a `BlockObserver` to invoke the `OperationQueueDelegate` method.
          let delegate = BlockObserver(startHandler: nil, produceHandler: { [weak self] (_, operation) in
            self?.addOperation(operation)
          }) { [weak self] (operation, errors) in
            if let q = self {
              q.delegate?.operationQueue?(operationQueue: q, operationDidFinish: operation, withErrors: errors)
            }
          }
