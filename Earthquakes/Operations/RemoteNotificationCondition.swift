...
private let RemoteNotificationQueue = AppleOperationQueue()
...
struct RemoteNotificationCondition: OperationCondition {
    ...
    static func didReceiveNotificationToken(token: NSData) {
      NotificationCenter.default.post(name: NSNotification.Name.init(RemoteNotificationName), object: nil, userInfo: ["token": token])
    }
    
    static func didFailToRegister(error: NSError) {
      NotificationCenter.default.post(name: NSNotification.Name.init(RemoteNotificationName), object: nil, userInfo: [
            "error": error
        ])
    }
...
*/
fileprivate class RemoteNotificationPermissionOperation: AppleOperation {
    let application: UIApplication
  ...
  fileprivate init(application: UIApplication, handler: @escaping (RemoteRegistrationResult)  -> Void) {
    ...
    override func execute() {
      DispatchQueue.main.async {
        ...
        notificationCenter.addObserver(self, selector: #selector(self.didReceiveResponse(notification:)), name: NSNotification.Name(rawValue: RemoteNotificationName), object: nil)
