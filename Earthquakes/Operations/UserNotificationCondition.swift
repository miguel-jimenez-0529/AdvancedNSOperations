        ...
        case (let current?, let settings) where current.contains(settings: settings):
            ...
            default:
                let error = NSError(code: .ConditionFailed, userInfo: [
                  ...
                  type(of: self).currentSettings as NSObject: current ?? NSNull(),
                  type(of: self).desiredSettings as NSObject: settings
                ])
...
*/
private class UserNotificationPermissionOperation: AppleOperation {
    let settings: UIUserNotificationSettings
    let application: UIApplication
